...
if __name__ == "__main__":
    ...
    for i in range(n_warmup):
        ...
        if a_warmup[i] == 1: # If AUDIT
            if is_fraud == 1:
                # Flat Detection Bonus (+5.0) plus the Utility
                r_warmup[i] = 5.0 + utility
            else:
                # Proportional Penalty for wasting auditor time
                r_warmup[i] = -0.5 * utility
                
        else: # If AUTO-APPROVE
            if is_fraud == 1:
                # The algorithm missed a fraud. MUST penalize it so it learns not to do this!
                # Penalize it based on how much money it lost BPJS.
                r_warmup[i] = -1.0 * utility 
            else:
                # Correctly approved a clean claim. Small positive reinforcement.
                r_warmup[i] = 0.5
    ...
    for t in range(n_live):
        ...
        # The agent simply predicts the best action based on Expected Reward + Uncertainty
        ...
        if action == 1:
            ...
            if is_fraud == 1:
                # FIX 1: Flat Detection Bonus (+5.0) plus the Utility
                reward = 5.0 + utility
                
                frauds_caught += 1
                total_utility_saved += utility
            else:
                # FIX 2: Proportional Penalty (Wasting time on a small claim is less punishing)
                # If utility is 1, penalty is -0.5. If utility is 8, penalty is -4.0.
                reward = -0.5 * utility
            ...
            reward = 0.0
            if is_fraud == 1:
                # SECRET TRACKER: AI missed a fraud! 
                missed_frauds += 1
                # Regret = The money BPJS lost because the algorithm failed to audit it.
                running_regret += utility 
            
        # Record the cumulative regret at this exact time step
